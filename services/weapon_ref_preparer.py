...
from services.weapon_crop_extractor import (
...
def prepare_compensated_crop(
    crop_bgr: np.ndarray,
    target_size: int = 256,
) -> tuple[np.ndarray, np.ndarray, dict]:
    crop_scaled = resize_long_side_if_needed(crop_bgr)
    object_bgra, info = extract_object_with_info_from_crop(crop_bgr)
...
def prepare_compensated_claymore_crop(
    crop_bgr: np.ndarray,
    target_size: int = 256,
) -> tuple[np.ndarray, np.ndarray, dict]:
    return prepare_compensated_crop(crop_bgr, target_size=target_size)
...
def clean_reference_alpha(object_bgra: np.ndarray) -> np.ndarray:
...
def prepare_generic_reference(ref_image: np.ndarray) -> tuple[np.ndarray, dict]:
    ref_bgra = ensure_bgra_image(ref_image)
    return clean_reference_alpha(ref_bgra), {
        "mode": "generic_alpha_clean",
    }
...
def prepare_weapon_reference(
    ref_image: np.ndarray,
    weapon_type: str,
    extra_scale: float = REF_EXTRA_SCALE,
) -> tuple[np.ndarray, dict]:
    ref_bgra = ensure_bgra_image(ref_image)
    ...
    if weapon_type == "Sword":
        ...
        cut_ratio = reframer.select_ref_cut_ratio(ref_bgra)
        ref_cut = cut_ref_bottom_percent(ref_bgra, cut_ratio)
        return reframer.reframe(ref_cut, extra_scale=extra_scale), {
            "mode": "sword_calibrated_geometry",
            "cut_ratio": float(cut_ratio),
            "extra_scale": float(extra_scale),
        }

    if weapon_type == "Claymore":
        reframer = ClaymoreReferenceReframer()
        reframed, params = reframer.reframe_window_autonomous(ref_bgra)
        return reframed, {
            "mode": "claymore_window_autonomous",
            "params": params,
        }
    ...
    if weapon_type == "Polearm":
        ...
        cut_ratio = float(params["cut"])
        ref_cut = cut_ref_bottom_percent(ref_bgra, cut_ratio)
        return reframer.reframe(ref_cut, extra_scale=extra_scale, params=params), {
            "mode": "polearm_calibrated_geometry",
            "cut_ratio": float(cut_ratio),
            "extra_scale": float(extra_scale),
            "params": params,
        }

    if weapon_type in ("Bow", "Catalyst"):
        return prepare_generic_reference(ref_bgra)

    raise NotImplementedError(f"Unsupported weapon type for prepared refs: {weapon_type}")
